...
export default function PageThumbnails() {
  ...
  // Follow the main viewer's scroll position: keep the current page's
  // thumbnail visible, nudging the sidebar's own scroll only when it would
  // otherwise fall outside the visible range (never fights a manual scroll
  // within that range).
  let lastFollowedPage = -1;
  createEffect(() => {
    ...
    if (page === lastFollowedPage) return;
    ...
    if (top < current) {
      ...
      scroller.scrollTop = top + height - viewport + 12;
    }
  });
